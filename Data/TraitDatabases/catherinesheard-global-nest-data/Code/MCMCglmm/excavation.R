...
gelmanprior<-list(B=list(mu=rep(0,8), #mu has to be the number of parameters to be estimated (k-1 for each category of size k, 1 for each continuous variable, 1 for the intercept) 
                         V=gelman.prior(~zMass+zHWI+zLat+zTemp+zTR+zPrecip+zPR, #you'll have to adjust this equation to match your linear model, and mu above may have to change
                                        data = x,  scale=1+pi^2/3)), 
                  R=list(V=1,fix=1),G=list(G1=list(V=1E-10,nu=-1)))

mod<-MCMCglmm(as.factor(Str.Excavation)~zMass+zHWI+zLat+zTemp+zTR+zPrecip+zPR,
              random=~PhyloName, 
...
save(Final.disp,file="Str-Ex.Rdata")
