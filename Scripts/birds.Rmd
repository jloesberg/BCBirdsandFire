...
dtrait <- dtrait %>% 
  ...
  select(name, synonyms, proj_id, et_id, synonyms, mlife, matage,nest, nsbank, nsgroun, nsgrass, nsclif, nsbuild, nsshru, nssnag, nscact, nstang, nsfloat, ntree, attem, eng_name,nest.tree.use, live_tree_nesting_use, snag_nesting_use, fmhawk, fmfogl, fmbagl, fstimb, fcinvert) %>% 
  ...
  mutate(index_diet = ifelse(trophic_niche == "Invertivore"|trophic_niche == "Nectarivore"|trophic_niche == "Granivore"|trophic_niche=="Frugivore", 1, 
         ...
         index_nesting = ifelse(nest_type == "cavity"| most_common_nesting_location == "snag", 1, 
                                if_else(most_common_nesting_location == "tree", -1, #negaitvely impacted if they nest in live trees
                                        ifelse(most_common_nesting_location=="grass"|most_common_nesting_location == "shrub", 1, 0))), #not including ground as a postive
         index_sum = rowSums(across(starts_with("index")), na.rm = TRUE))
...
ggsave(here("Figures", "index.first.try.png"), plot = index1, width = 40, height =20)
ggsave(here("Figures", "index.first.try.picidae2.png"), plot = woodpeckers, width = 8, height =6)
ggsave(here("Figures", "index.first.try.tyrannidae2.png"), plot = tyran, width = 8, height =6)
