...
dtrait <- read.csv(here("Data", "TraitDatabases", "Dubovyk", "Dubovyk_trait_database.csv")) #> nest <- dtrait %>% select(name, eng_name, nest, nsbank, nsgroun, nsgrass, nsclif, nsbuild, nsshru, nsdeci, nsconi, nssnag, nsfloat, nscact, nstang, ntree, nesting.location)
...
#note - here grass nesting is included in ground nesting
...
bctraits.all$ReproAge[bctraits.all$common_name=="Pacific Wren"] <- 1 #these are from winter wren
...
#bctraits.all$fstimb[bctraits.all$common_name=="Pacific Wren"] <- 10
# bctraits.all$fcinvert[bctraits.all$common_name=="Pacific Wren"] <- 60
...
#bctraits.all$ForStrat.midhigh[bctraits.all$common_name=="Pacific Wren"] <- 0
...
ex. wood duck:
https://www.audubon.org/field-guide/bird/wood-duck (fire would remove forest, causing negative impact on birds)
