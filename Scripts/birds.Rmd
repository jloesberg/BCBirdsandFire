...
birdlist <- read.csv(here("Data", "BirdLists", "bc_birds_checklist.csv"), na.strings = c("", "NA"))
birdlist <- birdlist[rowSums(is.na(birdlist)) != ncol(birdlist), ] #take out any blank rows
birdlist$Family <- sub("^\\s+", "", birdlist$Family) #take out spaces in the family column

#remove spp that dont really occur in BC
remove_species <- c(
  "American Black Duck",
  "Acorn Woodpecker",
  "Blue-headed Vireo",
...
#remove orders for which we dont expect fire responses
remove_orders <- c("CHARADRIIFORMES", "PODICIPEDIFORMES", "PROCELLARIIFORMES", "SULIFORMES", "PELECANIFORMES")
#filtering out the rare etc species
birdlist_filtered <- birdlist %>% 
  filter(is.na(status) | status %in% c("Vulnerable", "Endangered", "Near-threatened")) %>% 
  filter(!common_name %in% remove_species)%>% 
  filter(!Order %in% remove_orders)
#save this and the original before adding trait data
write.csv(birdlist, here("Data", "cleaned", "BC_birdlist_avibase.csv"), row.names = FALSE)
write.csv(birdlist_filtered, here("Data", "cleaned", "BC_birdlist_filtered.csv"), row.names = FALSE)

birdlist <- birdlist_filtered
remove(birdlist_filtered)
```

```{r read in available databases}
dtrait <- read.csv(here("Data", "TraitDatabases", "Dubovyk", "Dubovyk_trait_database.csv")) 
dtrait2 <- dtrait %>% 
  select(name, eng_name, 39:96, -fcfish, -fpdiur, -fpnoct, -klepto)

#clean the database up
dtrait <- dtrait %>% 
  mutate(nstree = nsdeci+nsconi,
         any_tree_nest = nstree+nssnag,  #to what scale do they use trees to nest? Essentially adding the % they use deciduous, coniferous, and snags
         nsgroun = nsgroun+nsgrass) %>% 
  select(name, synonyms, proj_id, et_id, synonyms, mlife, matage,nest, nsbank, nsgroun, nsclif, nsbuild, nsshru, nssnag, nscact, nstang, nsfloat, nstree, attem, eng_name,any_tree_nest, fmhawk, fmfogl, fmbagl, fstimb, nsdeci,nsconi) %>% 
  rename(Elton_ID = et_id,lifespan = mlife, ReproAge = matage,  nest.type = nest, clutches.per.year = attem,
         bank_nest = nsbank, ground_nest = nsgroun, cliff_nest = nsclif, human.structure_nest = nsbuild, shrub_nest = nsshru, snag_nest = nssnag, float_nest = nsfloat, cactus_nest = nscact, tangle_nest = nstang, live_tree_nest = nstree, conifer_nest = nsconi, deciduous_nest = nsdeci, for_hawking = fmhawk, for_foliage_gleaning =fmfogl, for_bark_gleaning = fmbagl) 
...
cols_to_check <- c("bank_nest", "ground_nest", 'cliff_nest', 'human.structure_nest', "shrub_nest", "float_nest", "snag_nest", 'cactus_nest', "tangle_nest", "live_tree_nest", "any_tree_nest")
dtrait <- dtrait %>%
    mutate(majority.nesting.location = ifelse(ground_nest>51, "ground", if_else(shrub_nest>51, "shrub", if_else(live_tree_nest>51, "tree", if_else(snag_nest>51, "snag", if_else(bank_nest>51, "bank", if_else(cliff_nest>51, "cliff", if_else(human.structure_nest>51, "human.structure", if_else(tangle_nest>51, "tangle", if_else(float_nest>51, "float", if_else(any_tree_nest>51, "tree", NA))))))))))) %>%
  rowwise() %>%
  mutate(row_values = list(c_across(all_of(cols_to_check))), sorted_indices = list(order(row_values, decreasing = TRUE)),
    most.common.nesting.location = cols_to_check[sorted_indices[1]], second.nesting.location = ifelse(
      row_values[sorted_indices[2]] == 0,
      NA_character_, 
      cols_to_check[sorted_indices[2]])) %>% 
    ungroup() %>%
 select(-c(row_values, sorted_indices))
###########################################################################################
#Ecology data paper - ELTON TRAITS
eco_t <- read.delim(here("Data", "TraitDatabases","EltonTraits", "Ecology_BirdFuncDat.txt"), sep = "\t", header = T)
eco_t <- eco_t %>% 
  mutate(ForStrat.water = ForStrat.watbelowsurf+ForStrat.wataroundsurf,
          ForStrat.canopy = ForStrat.midhigh+ ForStrat.canopy) %>% 
  select(SpecID, Scientific, English, Diet.Inv, Diet.Nect, Diet.Seed, starts_with("ForStrat"), -c(ForStrat.Source, ForStrat.SpecLevel, ForStrat.EnteredBy, ForStrat.watbelowsurf, ForStrat.wataroundsurf, ForStrat.midhigh)) %>% 
...
eco_t <- eco_t %>%
  ...
  mutate(row_values = list(c_across(all_of(cols_to_check))), sorted_indices = list(order(row_values, decreasing = TRUE)),
    ...
    second.foraging.location = ifelse(
      ...
      cols_to_check[sorted_indices[2]])) %>% 
    ungroup() %>%
  select(-c(row_values, sorted_indices, ForStrat.aerial, ForStrat.water))
...
de.trait <- full_join(eco_t, dtrait, by = c("Elton_ID", "eng_name")) %>% 
   mutate(Scientific = coalesce(Scientific, name))
  
#match to just bc birds
bctrait <- left_join(birdlist, de.trait, by = c("sci_name" = "Scientific"))

#did any of these not match?
unmatched <- bctrait %>%
  filter(is.na(Elton_ID)) %>% 
  select(names(birdlist))
#for those that didnt match, try common name instead of scientific
...
#for avonet, there are different spp in AVONET 1 and 3
avtrait <- avtrait %>% 
...
#add this into the original BC trait df
...
bctraits.all <- bctraits.all %>% 
  ...
  select(Order, Family, sci_name, common_name, status, mass_g, Habitat, Habitat.Density, Migration, Trophic.Level, Trophic.Niche, Primary.Lifestyle, Range.Size, lifespan, ReproAge, clutches.per.year, Kipps.Distance, Hand.Wing.Index, nest.type, majority.nesting.location, most.common.nesting.location, second.nesting.location, nest.category, any_tree_nest, ground_nest, shrub_nest, snag_nest, live_tree_nest, conifer_nest, deciduous_nest, majority.foraging.location, most.common.foraging.location, second.foraging.location, AV_ID, Elton_ID, D_ID, Elton_common_name, Elton_sci_name, Elton_syn, D_ID, D_sci_name,  for_hawking, for_foliage_gleaning, for_bark_gleaning, fstimb, Diet.Inv, Diet.Nect, Diet.Seed, ForStrat.ground, ForStrat.understory, ForStrat.canopy)
...
#lots of missing data for Pacific Wren - associated with old growth forests AND was recently separated from winter wren. So where there isnt data for pacific, use data from winter wren
bctraits.all$lifespan[bctraits.all$common_name=="Pacific Wren"] <- 4 
bctraits.all$clutches.per.year[bctraits.all$common_name=="Pacific Wren"] <- 1
...
bctraits.all$any_tree_nest[bctraits.all$common_name=="Pacific Wren"] <- 50
bctraits.all$live_tree_nest[bctraits.all$common_name=="Pacific Wren"] <- 20
bctraits.all$snag_nest[bctraits.all$common_name=="Pacific Wren"] <- 30
...
bctraits.all$ReproAge[bctraits.all$common_name=="Pacific Wren"] <- 1
bctraits.all$for_hawking[bctraits.all$common_name=="Pacific Wren"] <- 0
bctraits.all$for_foliage_gleaning[bctraits.all$common_name=="Pacific Wren"] <- 40
bctraits.all$for_bark_gleaning[bctraits.all$common_name=="Pacific Wren"] <- 0
bctraits.all$conifer_nest[bctraits.all$common_name=="Pacific Wren"] <- 10
bctraits.all$deciduous_nest[bctraits.all$common_name=="Pacific Wren"] <- 10
bctraits.all$ForStrat.ground[bctraits.all$common_name=="Pacific Wren"] <- 50
bctraits.all$ForStrat.understory[bctraits.all$common_name=="Pacific Wren"] <- 50
bctraits.all$ForStrat.canopy[bctraits.all$common_name=="Pacific Wren"] <- 0
bctraits.all$Diet.Inv[bctraits.all$common_name=="Pacific Wren"] <- 60
bctraits.all$Diet.Nect[bctraits.all$common_name=="Pacific Wren"] <- 0
bctraits.all$Diet.Seed[bctraits.all$common_name=="Pacific Wren"] <- 10
bctraits.all$ground_nest[bctraits.all$common_name=="Pacific Wren"] <- 20
bctraits.all$shrub_nest[bctraits.all$common_name=="Pacific Wren"] <- 0
```

```{r}
#adding in data from birdbase
#BIRDBASE
birdbase <- read.csv(here("Data", "TraitDatabases", "Birdbase", "BIRDBASE.database.csv"), skip = 1)
#get the traits were interested in:
birdbase <- birdbase %>% 
  clean_names()
birdbase <- birdbase %>% 
...
birdbase$forest.use[is.na(birdbase$forest.use)] <- 0 #is NA for forest use, make it 0

#just get BC birds
birdbase.bc <- left_join(birdlist, birdbase, by = c("sci_name" = "latin"))
#if sci name didnt match, use common name:
unmatched.i <- birdbase.bc %>%
  filter(is.na(ioc_15_1)) %>% #this is a unique ID for birdbase, if its NA here, it didnt match with the BC list
  select(names(birdlist))
matched_by_i<- unmatched.i %>%
  left_join(birdbase, by = c("common_name" = "common.name"))
#if common name also didnt match, use the former scientific name:
...
bctraits.all <- bctraits.all %>% 
  select(Order, Family, sci_name, common_name, iucn.2024, status, mass_g, Habitat, Habitat.Density, Migration, Trophic.Level, Trophic.Niche, Primary.Lifestyle, Range.Size, lifespan, ReproAge, clutches.per.year, Kipps.Distance, Hand.Wing.Index, nest.type, majority.nesting.location, most.common.nesting.location, second.nesting.location, nest.category, any_tree_nest, ground_nest, shrub_nest, snag_nest, live_tree_nest, conifer_nest, deciduous_nest, majority.foraging.location, most.common.foraging.location, second.foraging.location, AV_ID, Elton_ID, D_ID, Elton_common_name, Elton_sci_name, Elton_syn, D_ID, D_sci_name,  for_hawking, for_foliage_gleaning, for_bark_gleaning, fstimb, Diet.Inv, Diet.Nect, Diet.Seed, ForStrat.ground, ForStrat.understory, ForStrat.canopy, elevational_range, forest.use, primary_habitat, habitat.breadth, primary_diet,diet.breadth, Eco.Spec.Index)

#these are old growth associated spp
...
missing_names <- old_growth[!(old_growth %in% bctraits.all$common_name)] #do all these names appear in the BC list?

bctraits.all <- bctraits.all %>%
  mutate(old_growth = if_else(common_name %in% old_growth, 1, 0))

#now clean it and save it:
bctraits.all <- bctraits.all %>% 
  janitor::clean_names()
...
"habitat_breadth" - the number of major habitats used. Lower = more habitat specialized, higher = broader habitat use [birdbase]
 ...
 fmhawk: foraging method hawking, fmfogl: foraging method foliage gleaning, fmbagl: foraging method bark gleaning, fstimb: foraging substrate: timber, fcinvert: foraging category: invertebrate [%'s]
...
"ForStrat-ground": Prevalence of: Foraging on ground - estimated % use (categories sum to 100% total) [eltontraits]

"ForStrat-understory": Prevalence of: Foraging below 2m in understory in forest, forest edges, bushes or shrubs:estimated % use (categories sum to 100% total) [eltontraits]

"ForStrat-midhigh": Prevalence of: Foraging in mid to high levels in trees or high bushes (2m upward), but below canopy, estimated % use (categories sum to 100% total) [eltontraits]

"ForStrat-canopy": Prevalence of: Foraging in or just above (from) tree canopy. estimated % use (categories sum to 100% total) [eltontraits]

"old_growth": are they associated with old growth? list from Peter Arcese