...
dtrait <- dtrait %>%
    mutate(majority.nesting.location = if_else(ground>51, "ground", if_else(grass>51, "grass", if_else(shrub>51, "shrub", if_else(tree>51, "tree", if_else(snag>51, "snag", if_else(bank>51, "bank", if_else(cliff>51, "cliff", if_else(human.structure>51, "human.structure", if_else(tangle>51, "tangle", if_else(float>51, "float", NA))))))))))) %>% #if there is a majority (>51%) of nesting location
  rowwise() %>%
  mutate(
...
bctraits.all <- bctraits.all %>% 
  ...
  select(Order, Family, sci_name, common_name, status, mass_g, Habitat, Habitat.Density, Migration, Trophic.Level, Trophic.Niche, Primary.Lifestyle, Range.Size, Diet.5Cat, lifespan, ReproAge, clutches.per.year, nest.type, majority.nesting.location, most.common.nesting.location, second.nesting.location, nest.category, redlistCategory, populationTrend, AV_ID, Elton_ID, D_ID, Elton_common_name, Elton_sci_name, Elton_syn, D_ID, D_sci_name)
